...
class AssemblyExport implements FromView, WithEvents
{
    ...
    public function view(): View
    {
        ...
        return view('exports.assembly', [
            'items' => $this->items->map(function ($item) use ($dicts) {
                ...
                $item->status = $dicts['assembly_status']->where('value', $item->status)->value('name');
                $item->casts['type'] = 'string';
                $item->casts['line'] = 'string';
                $item->casts['plant'] = 'string';
                $item->casts['status'] = 'string';
                return $item;
            })
        ]);
    }
