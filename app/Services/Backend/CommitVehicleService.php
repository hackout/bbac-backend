...
class CommitVehicleService extends Service
{
    ...
    public function getList(User $user, array $data): array
    {
        ...
        $result = parent::list();
        $result['items'] = $result['items']->map(function (CommitVehicle $item) {
            return [
                'id' => $item->id,
                'name' => $item->name,
                ...
                'can_approve' => $item->items->count() > 0 && $item->status == CommitVehicle::STATUS_DRAFT,
            ];
        });
        return $result;
    }
    ...
    {
        Excel::import(new CommitVehicleImport($user, $file), $file);
    }
