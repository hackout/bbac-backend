...
class Service
{
        ...
        if ($data) {
            ...
            foreach ($conditions as $key => $value) {
                ...
                if ($action == 'datetime_range' && array_key_exists($key, $data) && $data[$key]) {
                    ...
                    if (is_array($fields)) {
                        throw ValidationException::withMessages(['field_error' => __('admin.validation.common.field_error')]);
                    }
                    if ($datetime[0]) {
                ...
                if ($relation === $key) {
                    if (strpos($relation, ':') === false) {
                        ...
                        if (!$relationColumnString)
                            throw ValidationException::withMessages(['column.incorrect' => '选项语法错误']);
    ...
    public function update(string|int $id, array $data)
    {
        ...
        if (!$this->item) {
            throw ValidationException::withMessages(['update_fail' => __('admin.common.update_fail')]);
        }
        $this->item->fill($data);
        $result = $this->item->save();
        if ($result) {
    ...
    public function delete(string|int $id)
    {
        $this->item = $this->model->where($this->primaryKey, $id)->first();
        if (!$this->item) {
            throw ValidationException::withMessages(['delete_fail' => __('admin.common.delete_fail')]);
        }
