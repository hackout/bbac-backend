...
trait ImportTemplateTrait
{
    ...
    public function downloadImportTemplate(): BinaryFileResponse
    {
        if (!file_exists($this->getImportTemplate())) {
            throw ValidationException::withMessages(['file.error' => '模板不存在']);
        }
    ...
    {
        return property_exists($this, 'import') && $this->import ? $this->import : '\App\Imports\\' . basename($this->className) . 'Import';
    }
}