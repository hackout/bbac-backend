            ...
            <el-table :data="items" border v-loading="loading" height="600px" stripe highlightCurrentRow>
                ...
                <el-table-column label="文件名称" align="center">
                    <template #default="scope">
                        ...
                        <el-upload v-else :action="$route('document.assembling_update',{engine: scope.row.engine})" :ref="`importUploadLink_${scope.row.engine}`"
                            :on-success="importSuccess" class="page-search-buttons-upload" :limit="1" :show-file-list="false"
                ...
                <el-table-column label="操作" align="center" prop="action" width="185" fixed="right">
                    <template #default="scope">
                        <el-upload :action="$route('document.assembling_update',{engine: scope.row.engine})" :ref="`importUpload_${scope.row.engine}`"
                            :on-success="importSuccess" class="page-search-buttons-upload" :limit="1" :show-file-list="false"
                            :headers="uploadHeaders">
...
export default {
    ...
    methods: {
        async getData() {
            this.loading = true
            var res = await this.$axios.get(this.$route('document.list', { type: 2 }));
            this.loading = false
