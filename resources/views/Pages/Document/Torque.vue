<template>
    <Layout>
        ...
        <div class="page-block DataTable">
            <el-table :data="items" v-loading="loading" border height="600px" stripe highlightCurrentRow>
                ...
                <el-table-column label="文件名称" align="center">
                    <template #default="scope">
                        <el-link :href="scope.row.url" type="primary" size="small" target="_blank"
                            v-if="scope.row.url">{{ scope.row.name
                            }}</el-link>
                        <el-upload v-else :action="$route('document.torque_update', { engine: scope.row.engine })"
                ...
                <el-table-column label="操作" align="right" prop="action" width="185" fixed="right">
                    <template #default="scope">
                        <el-upload :action="$route('document.torque_update', { engine: scope.row.engine })"
...
export default {
    ...
    methods: {
        async getData() {
            this.loading = true
            var res = await this.$axios.get(this.$route('document.list', { type: 3 }));
            this.loading = false
