<template>
    <el-dialog title="送审批" v-model="visitable" width="550px" @closed="$emit('closed')">
        <el-form :model="form" v-loading="loading" :rules="{}" label-width="150px" ref="form" @submit.native.prevent="onSave">
            ...
            <el-form-item label="版本变动内容" required>
            ...
            <el-form-item label="变更内容" prop="content" :rules="{ required: true, message: '变更内容不能为空', trigger: 'blur' }">
            ...
            <el-form-item label="影响范围" prop="influence"
                :rules="{ required: true, message: '影响范围不能为空', trigger: 'blur' }">
            ...
            <el-form-item label="关注事项" prop="concerns"
                :rules="{ required: true, message: '关注事项不能为空', trigger: 'blur' }">
    ...
    methods: {
        onSave() {
            this.$refs.form.validate(async (valid) => {
                if (valid) {
                    this.loading = true
                    const res = await this.$axios.post(this.$route('commit_approve.create',{id:this.item.id}),this.form)
                    this.loading = false;
        ...
        async getData(){
            this.loading  = true;
            const res = await this.$axios.get(this.$route('commit.changed',{id:this.item.id}))
            this.loading = false
            if(res.code == this.$config.successCode)
            {
