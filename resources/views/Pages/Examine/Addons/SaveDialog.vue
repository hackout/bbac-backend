<template>
    <el-dialog :title="titles[mode]" v-model="visitable" width="550px" @closed="$emit('closed')">
        <el-form :model="form" :rules="rules" label-width="100px" ref="form" @submit.native.prevent="onSave">
            ...
            <el-form-item label="发动机机型" prop="engine">
                <el-select v-model="form.engine" style="width:100%;" placeholder="请选择发动机机型" @change="getVersion"
                    clearable>
            ...
            <el-form-item label="上一个版本" prop="parent_id" v-if="mode == 'add'">
                <el-select v-model="form.parent_id" @change="changeParent" style="width:100%;" placeholder="请选择上一个版本" clearable>
                    <el-option label="全新版本" value=""></el-option>
                    <el-option v-for="(item, index) in commits" :key="index" :label="`${item.name}(${item.version})`"
                        :value="item.value"></el-option>
                </el-select>
            </el-form-item>
...
export default {
    ...
    methods: {
        async getVersion() {
            let res = await this.$axios.get(this.$route('commit.option'), { engine: this.form.engine, type: this.type, sub_type: this.form.sub_type })
        ...
        onSave() {
            this.$refs.form.validate(async (valid) => {
                if (valid) {
                    this.loading = true
                    const url = this.mode == 'add' ? this.$route('commit.create', { type: this.type }) : this.$route('commit.update', { id: this.form.id })
