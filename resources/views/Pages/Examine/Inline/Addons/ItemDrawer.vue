<template>
    <el-drawer :title="`${item.name} 考核项列表`" v-model="visitable" size="calc(100vw - 400px)" @closed="$emit('success')">
        <el-table border v-loading="loading" class="DataTable" :data="items">
            ...
            <el-table-column prop="type" align="center" label="检查类型" width="135">
                <template #default="scope">
                    <el-tag size="small">{{ $status('examine_inline_item_type', scope.row.type) }}</el-tag>
                </template>
            </el-table-column>
        ...
        <el-dialog modal-class="chooseDialog" v-model="chooseAddVisit" :show-close="false">
            <el-row :gutter="20">
                <el-col :span="24 / autoType.length" v-for="(t, i) in autoType" :key="i">
        ...
        <ItemDetail ref="ItemDetail" v-if="viewable" @success="getItems" @closed="viewable = false"
            :examine_inline_item_type="examine_inline_item_type"></ItemDetail>
        <ItemDialog ref="ItemDialog" v-if="editable" @success="getItems" @closed="editable = false" :special="special"
            :bolt_status="bolt_status" :bolt_model="bolt_model" :bolt_type="bolt_type"
            :examine_inline_item_type="examine_inline_item_type"></ItemDialog>
        <ItemOption ref="ItemOption" @closed="optionVisit = false" @success="getItems" v-if="optionVisit"></ItemOption>
...
export default {
    ...
    computed: {
        autoType() {
            let nowType = types[this.item.type - 1]
            return this.examine_inline_item_type.filter(n => nowType.indexOf(n.value) > -1)
        }
    },
    methods: {
        async getItems() {
            this.loading = true
            let res = await this.$axios.get(this.$route('commit_inline_item.list', { id: this.item.id }))
            this.loading = false
        ...
        editItem(item) {
            this.editable = true
            this.$nextTick(() => {
                this.$refs.ItemDialog.open('edit', item.type, this.item, item)
            })
        },
        openItem(type) {
            ...
            this.editable = true
            this.$nextTick(() => {
                this.$refs.ItemDialog.open('add', type, this.item)
            })
        },
        openOption(item) {
            this.optionVisit = true
            this.$nextTick(() => {
                this.$refs.ItemOption.open(this.item, item)
            })
        },
        toTop(index) {
        ...
        async deleteItem(item) {
            let res = await this.$axios.delete(this.$route('commit_inline_item.delete', { id: this.item.id, item_id: item.id }))
            if (res.code == this.$config.successCode) {
        ...
        async orderItem(items) {
            let res = await this.$axios.post(this.$route('commit_inline_item.order', { id: this.item.id }), { list: items.map(n => n.id) })
            if (res.code == this.$config.successCode) {
                items = items.map(n => {
