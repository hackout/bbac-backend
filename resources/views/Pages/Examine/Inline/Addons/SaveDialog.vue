...
export default {
    ...
    methods: {
        async getVersion() {
            let res = await this.$axios.get(this.$route('commit_inline.option'), { engine: this.form.engine })
        ...
        onSave() {
            this.$refs.form.validate(async (valid) => {
                if (valid) {
                    this.loading = true
                    const url = this.mode == 'add' ? this.$route('commit_inline.create') : this.$route('commit_inline.update', { id: this.form.id })
