<template>
    <el-drawer title="考核模板详情" v-model="visitable" :size="650" @closed="$emit('closed')">
        <el-descriptions title="基础信息" v-loading="loading" :column="1" border class="detail-box">
            ...
            <el-descriptions-item label="考核类型"><span>{{ $status('examine_product_item_type', item.type)
                    }}</span></el-descriptions-item>
            <el-descriptions-item label="考核机型"><span>{{ $status('engine_type', item.engine)
                    }}</span></el-descriptions-item>
            ...
            <el-descriptions-item label="备注信息"><span>{{ item.description }}</span></el-descriptions-item>
        </el-descriptions>
...
export default {
    ...
    methods: {
        ...
        async getData() {
            this.loading = true
            var res = await this.$axios.get(this.$route('commit_vehicle.detail', { id: this.commit.id }))
            this.loading = false
