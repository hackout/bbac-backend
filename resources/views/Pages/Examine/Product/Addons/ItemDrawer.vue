<template>
    <el-drawer :title="`${item.name} 考核项列表`" v-model="visitable" size="calc(100vw - 400px)" @closed="$emit('success')">
        ...
        <el-dialog modal-class="chooseDialog" v-model="chooseAddVisit" :show-close="false">
            <el-row :gutter="20">
                <el-col :span="24 / examine_product_item_type.length" v-for="(t, i) in examine_product_item_type"
                    :key="i">
        ...
        <ItemDetail ref="ItemDetail" v-if="viewable" @success="getItems" @closed="viewable = false"
            :examine_product_item_type="examine_product_item_type"></ItemDetail>
        <ItemDialog ref="ItemDialog" v-if="editable" @success="getItems" @closed="editable = false" :parts="parts"
            :examine_product_item_type="examine_product_item_type"></ItemDialog>
...
export default {
    ...
    methods: {
        async getItems() {
            this.loading = true
            let res = await this.$axios.get(this.$route('commit_product_item.list', { id: this.item.id }))
            this.loading = false
        ...
        async deleteItem(item) {
            let res = await this.$axios.delete(this.$route('commit_product_item.delete', { id: this.item.id, item_id: item.id }))
            if (res.code == this.$config.successCode) {
        ...
        async orderItem(items) {
            let res = await this.$axios.post(this.$route('commit_product_item.order', { id: this.item.id }), { list: items.map(n => n.id) })
            if (res.code == this.$config.successCode) {
                items = items.map(n => {
