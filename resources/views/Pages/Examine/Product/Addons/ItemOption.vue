<template>
    <el-dialog title="实际测量项列表" v-model="visitable" width="800px" @closed="$emit('closed')">
        ...
        <el-table v-loading="loading" class="DataTable" :data="items" border style="width: 100%;margin-top: 5px">
            <el-table-column label="实际测量项" prop="name">
                <template #default="scope">
                    <el-input v-model="items[scope.$index].name" @dblclick="handleEdit(scope.$index)"
                        :readonly="!scope.row.editable" placeholder="实际测量项"></el-input>
                </template>
            </el-table-column>
...
export default {
    ...
    methods: {
        ...
        async onSave() {
            this.loading = true
            let data = this.items.map(n => {
                return { name: n.name, sort_order: n.sort_order }
            })
            let res = await this.$axios.post(this.$route('commit_product_item.option', { id: this.commit.id, item_id: this.item.id }), { items: data })
            this.loading = false
        ...
        open(commit, item) {
            this.commit = commit
            this.item = item
            this.items = this.$tool.objCopy(item.options).map(n=>{
                return {
                    name: n.name,
