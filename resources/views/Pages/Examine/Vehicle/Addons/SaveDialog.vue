...
export default {
    ...
    methods: {
        async getVersion() {
            let res = await this.$axios.get(this.$route('commit_vehicle.option'), { engine: this.form.engine })
        ...
        onSave() {
            this.$refs.form.validate(async (valid) => {
                if (valid) {
                    this.loading = true
                    const url = this.mode == 'add' ? this.$route('commit_vehicle.create') : this.$route('commit_vehicle.update', { id: this.form.id })
