<template>
    <Layout>
        ...
        <EngineDialog ref="EngineDialog" :examine_product_item_type="examine_product_item_type" v-if="EngineDialogVisit"
            @success="refreshData" @closed="EngineDialogVisit = false"></EngineDialog>
    </Layout>
</template>
<script>
import EngineDialog from './Addons/EngineDialog.vue';
export default {
    components: {
        EngineDialog
    },
    ...
    methods: {
        ...
        openDetail(item) {
            this.EngineDialogVisit = true
            this.$nextTick(() => {
                this.$refs.EngineDialog.open(item)
            })
        },
        refreshData() {
            this.editable = false
