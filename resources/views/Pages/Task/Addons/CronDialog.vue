<template>
    <el-dialog title="新增生产任务" v-model="visitable" width="850px" @closed="$emit('closed')">
        <el-form :model="form" ref="form" label-position="top" :rules="rules" v-loading="loading" label-width="120px"
            ...
            <div class="form-block-content">
                <el-row :gutter="20">
                    <template v-for="(item, index) in form.days" :key="index">
                        <el-col :span="18">
                            <el-form-item :rules="[{ required: true, message: '请填写排产日期', trigger: 'change' }]">
                                <el-date-picker style="width:100%;" v-model="form.days[index].date" type="daterange"
                                    range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" />
                            </el-form-item>
                        </el-col>
                        <el-col :span="6">
                            <el-button size="small" type="primary" @click="addItems"
                                icon="el-icon-circle-plus"></el-button>
                            <el-button size="small" v-if="index > 0" @click="deleteItems(index)" type="danger"
                                icon="el-icon-delete-filled"></el-button>
                        </el-col>
                        <el-col :span="9">
                            <el-form-item :rules="{ required: true, message: '请填写天数', trigger: 'blur' }">
                                <el-input type="number" v-model="form.days[index].day" placeholder="X天"
                                    clearable></el-input>
                            </el-form-item>
                        </el-col>
                        <el-col :span="9">
                            <el-form-item :rules="{ required: true, message: '请填写次数', trigger: 'blur' }">
                                <el-input type="number" v-model="form.days[index].unit" placeholder="X次"
                                    clearable></el-input>
                            </el-form-item>
                        </el-col>
                ...
                <el-row :gutter="20">
                    <el-col :span="9">
                        <el-form-item prop="yield">
                            <el-input type="number" v-model="form.yield" placeholder="X台排产" clearable></el-input>
                        </el-form-item>
                    </el-col>
                    <el-col :span="9">
                        <el-form-item prop="yield_unit">
                            <el-input type="number" v-model="form.yield_unit" placeholder="X次" clearable></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
            </div>
    ...
    data() {
        return {
            ...
            rules: {
                ...
                days: [
                    { required: true, type: "array", message: '请至少填写一个循环时间段', trigger: 'change' }
                ],
                yield: [
    ...
    methods: {
        ...
        onSave() {
            this.$refs.form.validate(async (valid) => {
                if (valid) {
                    this.saving = true
                    let res = await this.$axios.post(this.$route('task_cron.create'), this.form)
                    this.saving = false
                    if (res.code == this.$config.successCode) {
