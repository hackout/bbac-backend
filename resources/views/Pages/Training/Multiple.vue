        ...
        <div class="page-block">
            <div class="page-search">
                <div class="page-search-buttons">
                    ...
                    <el-upload :action="$route('training.import', { type: 'multiple' })" ref="importUpload"
                    ...
                    <el-button type="primary" @click="$goTo('training.template', { type: 'multiple' })" link
                ...
                <div class="page-upload-file">
                    <el-upload :action="itemUploadUrl" ref="importUpload_file" :on-success="importSuccess"
                        class="page-search-buttons-upload" :limit="9" multiple :show-file-list="false" :headers="uploadHeaders">
                        <el-button link icon="el-icon-upload" id="updateFile" type="primary">上传文件</el-button>
                    </el-upload>
                </div>
            </div>
            <DataTable ref="table" :apiName="$route('training.list', { type: 'multiple' })"
...
export default {
    ...
    methods: {
        uploadFile(item) {
            this.itemUploadUrl = this.$route('training.upload', { type: 'multiple', id: item.id })
        ...
        async deleteFile(id, uuid) {
            let res = await this.$axios.delete(this.$route('training.file_delete', { type: 'multiple', id: id, file: uuid }))
        ...
        async exportData() {
            var res = await this.$axios.post(this.$route('training.export', { type: 'multiple' }), this.query)
            if (res.code == this.$config.successCode) {
        ...
        deleteItem() {
            this.$confirm('确定删除所选项?', '操作提示').then(async () => {
                this.deleting = true
                var res = await this.$axios.post(this.$route('training.delete', { type: 'multiple' }), { ids: this.items.map(n => n.id) })
                this.deleting = false
                if (res.code == this.$config.successCode) {
        ...
        refreshData() {
            this.itemIds = []
            this.$refs.importUpload_file.clearFiles()
