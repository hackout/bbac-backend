        ...
        <div class="page-block">
            <div class="page-search">
                <div class="page-search-buttons">
                    ...
                    <el-upload :action="$route('training.import', { type: 'skill' })" ref="importUpload"
                    ...
                    <el-button type="primary" @click="$goTo('training.template', { type: 'skill' })" link
            ...
            <DataTable ref="table" :apiName="$route('training.list', { type: 'skill' })"
                ...
                <el-table-column label="培训内容" align="center" prop="name" width="200" fixed="left"></el-table-column>
                    ...
                    <el-table-column v-for="(column, index) in columns" :key="index" :label="column.name" align="center"
                        ...
                        <template #default="scope">
                            <el-dropdown @command="changeStatus(scope.row.id, column.value, $event)" trigger="click">
                                <el-text type="info" tag="span"
                                    v-if="scope.row.users[column.value] == '0'">设置</el-text>
                                <span class="percent" v-else-if="scope.row.users[column.value] != '4'">
                                    <component :is="icons[scope.row.users[column.value]]"></component>
                                </span>
                                <el-text type="info" tag="span" v-else>不涉及</el-text>
                                <template #dropdown>
                                    <el-dropdown-menu>
                                        <el-dropdown-item v-for="(ss, ii) in training_item_status" :key="ii"
                                            :command="ss.value">
                                            <el-text type="info" tag="span" v-if="ss.value == 'not_involve'">{{ ss.name
                                                }}</el-text>
                                            <el-text type="primary" tag="span" v-else>{{ ss.name }}</el-text>
                                        </el-dropdown-item>
                                    </el-dropdown-menu>
                                </template>
                            </el-dropdown>
...
export default {
    ...
    methods: {
        uploadFile(item) {
            this.itemUploadUrl = this.$route('training.upload', { type: 'skill', id: item.id })
        ...
        async deleteFile(id, uuid) {
            let res = await this.$axios.delete(this.$route('training.file_delete', { type: 'skill', id: id, file: uuid }))
        ...
        async exportData() {
            var res = await this.$axios.post(this.$route('training.export', { type: 'skill' }), this.query)
            if (res.code == this.$config.successCode) {
        ...
        async changeStatus(id, user_id, val) {
            var res = await this.$axios.post(this.$route('training.patch', {
                type: 'skill'
            }), {
                id: id,
                user_id: user_id,
        ...
        deleteItem() {
            this.$confirm('确定删除所选项?', '操作提示').then(async () => {
                this.deleting = true
                var res = await this.$axios.post(this.$route('training.delete', { type: 'skill' }), { ids: this.items.map(n => n.id) })
                this.deleting = false
                if (res.code == this.$config.successCode) {
